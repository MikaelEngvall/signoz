...
function StateTimelinePanelWrapper({
	...
	console.log('[StateTimeline] RAW response:', JSON.stringify({
	...
	// eslint-disable-next-line no-console
	console.log('[StateTimeline] Series detail:', queryData.map(qd => ({
	...
	// eslint-disable-next-line no-console
	console.log('[StateTimeline] widget.thresholds:', widget.thresholds, 'widget.id:', widget.id);
	...
	// eslint-disable-next-line no-console
	console.log('[StateTimeline] queryData:', queryData.length, 'series, timeRange:', timeRange, 'rows:', swimLaneModel.rows.length, 'thresholds:', thresholds.length);
	if (swimLaneModel.rows.length > 0) {
		// eslint-disable-next-line no-console
		console.log('[StateTimeline] First row:', swimLaneModel.rows[0].label, 'segments:', swimLaneModel.rows[0].segments.length, swimLaneModel.rows[0].segments[0]);
	}
	// eslint-disable-next-line no-console
	console.log('[StateTimeline] containerDimensions:', containerDimensions, 'panelWidth:', containerDimensions.width, 'panelHeight:', containerDimensions.height);
